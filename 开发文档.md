...
# 4.6 开始打包:(base) (20231201_pyGUI) PS D:\PythonProject\whisper_gui> pyinstaller build.spec    这个已经启用
# 4.7 开始打包:(base) (20231201_pyGUI) PS D:\PythonProject\whisper_gui>  pyinstaller build-obf.spec  如果使用这个打包要先自己生成混淆
4.8 开始打包(base) (20231201_pyGUI) PS D:\PythonProject\whisper_gui> ./build.bat    这个和bat脚本包含了混淆和打包
...
2. 移除check , 改成序号
3. 非模态窗体, 不用一直置顶
4. 每次都加载模型, 在批处理的时候, 是不是可以只加载一次模型呢? 
5. 隐藏文件列表窗体的  全选/反选  20251020_完成
6. 点击开始按钮的时候, 如果超过一个文件, 则弹出一个提示, 您选择了 {x} 个文件, 可能需要较长时间处理, 是否继续? 
7. ❌ SRT保存失败：没有可用的语音段落
